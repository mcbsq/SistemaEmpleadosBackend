...
Las contraseñas de CUENTAS son de prueba/desarrollo únicamente.
No ejecutar este script contra una base de datos de producción.
...
# ─── Conexión ────────────────────────────────────────────────────────────────
# Sin default: si falta la variable, el script debe fallar explícito en vez
# de usar una credencial real hardcodeada.
MONGO_URI = os.environ.get("MONGO_URI")
if not MONGO_URI:
    print("Falta MONGO_URI en el entorno. Define la variable antes de ejecutar seed.py.")
    sys.exit(1)
...
# Contraseñas de prueba — cámbialas si este script llega a correr fuera de dev.
CUENTAS = [
    {
...
def seed():
    ...
    for cuenta in CUENTAS:
        ...
        if s["admin_enabled"]:
            # Cuenta de login vive en Aegis; aquí solo enlazamos. Aegis genera
            # una contraseña TEMPORAL (la de CUENTAS se ignora en este modo) y
            # el usuario debe cambiarla en su primer inicio de sesión.
            created, err = aegis_admin_create_user(email)
            if err:
                body, status = err
            ...
            if not aegis_id:
                ...
                continue
            cuenta["temp_password"] = created.get("temp_password")
        ...
        print(f"     Role:        {role}")
        if s["admin_enabled"]:
            print(f"     Password:    {cuenta.get('temp_password')} (TEMPORAL — cambiarla al primer login)")
        else:
    ...
    print("  ──────────────────────────────────────────────")
    for c in CUENTAS:
        pw = c.get("temp_password") or c["password"]
        print(f"  {c['user']:<18} {pw:<16} {c['role']}")
    if s["admin_enabled"]:
        print("\n  Login en la API: identifier = email completo o su parte local (sin @).")
        print("  Contraseñas temporales: el usuario debe cambiarlas en el primer login")
        print("  (POST /change-password de este backend o el flujo de tu app).\n")
    print()
